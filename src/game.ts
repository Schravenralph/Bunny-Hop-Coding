    ...
    this.movingPlatforms.forEach(platform => {
      if (this.bunny.x + this.bunny.width > platform.x &&
        ...
        if (platform.moveX !== false) {
          const moveSpeed = platform.moveSpeed || 1.5;
          const directionX = platform.directionX || 1;
          this.bunny.x += moveSpeed * directionX;
        }
      }
    });
  ...
  update(): void {
    ...
    this.movingPlatforms.forEach(platform => {
      ...
      if (platform.moveX !== false) {
        const moveSpeed = platform.moveSpeed || 1.5;
        const directionX = platform.directionX || 1;
        platform.x += moveSpeed * directionX;
        ...
        if (platform.x > startX + moveRange) {
          platform.directionX = -1;
          platform.x = startX + moveRange; // Clamp to boundary
      ...
      if (platform.moveY === true) {
        const moveSpeed = platform.moveSpeed || 1.5;
        const directionY = platform.directionY || 1;
        platform.y += moveSpeed * directionY;
        const startY = platform.originalY || platform.y;
        const moveRange = platform.moveRange || 100;
        // Reverse direction if out of range and clamp position
        if (platform.y > startY + moveRange) {
          platform.directionY = -1;
          platform.y = startY + moveRange; // Clamp to boundary
